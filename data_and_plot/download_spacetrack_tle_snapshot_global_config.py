...
TARGET_TIME_UTC = "2026-07-24 00:00:00"
...
def parse_utc_datetime(value: str) -> datetime:
    ...
    try:
        ...
        raise ValueError(
            f"无法解析时间 {value!r}，建议格式: YYYY-MM-DD HH:MM:SS") from exc
...
def select_latest_tles_at_or_before_target(
    ...
    with session.get(query_url, stream=True,
        ...
        if missing:
            raise RuntimeError(
                f"返回字段不完整，缺少: {sorted(missing)}；实际字段: {reader.fieldnames}")
    ...
    with tle_path.open("w", encoding="utf-8", newline="\n") as tle_file:
        ...
        for row in ordered:
            ...
            name = (row.get("OBJECT_NAME")
                    or f"NORAD {row['NORAD_CAT_ID']}").strip()
...
def main() -> None:
    ...
    if not username or not password:
        raise RuntimeError("未找到 Space-Track 账号密码。请设置环境变量 "
                           "SPACETRACK_USERNAME 和 SPACETRACK_PASSWORD。")
    ...
    session.headers.update({
        "User-Agent":
        "tle-snapshot-downloader/1.0",
        "Accept":
        "text/csv,application/json;q=0.9,*/*;q=0.8",
    })

    try:
        login(session, username, password)
        records = select_latest_tles_at_or_before_target(
            session, query_url, target)
    finally:
        session.close()
    ...
    csv_path, tle_path = write_outputs(records.values(), target,
                                       Path(OUTPUT_DIR))

    ages = [(target - row["_epoch_dt"]).total_seconds() / 3600.0

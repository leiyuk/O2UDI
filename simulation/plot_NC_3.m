...
for k = 1:4
    ...
    scatter(ax_left,x_plot, y_plot, 2, f, 'filled'); 
%     colorbar;
    colormap(viridis);
...
%     subtitle(['evolution time: ', num2str(round((pic_num-1)*Ts/86400)), ' day']);
    ...
    ax_right = nexttile(tl_child,2);
    x_plot = Y(1, 1:num_tuiyan)'; % 转置为列向量
    y_plot = Y(3, 1:num_tuiyan)'; % 转置为列向量    
    % --- 计算密度 ---
    % 这里的输入必须是 [num_true x 2] 的矩阵
    [f, ~] = ksdensity([x_plot, y_plot], [x_plot, y_plot]);    
    % --- 绘图 ---
...
outfile = fullfile(pwd,'fig1_3.png');  % 路径可自改

...
for k = 1:3
    ...
    plot(xiao_cankao(1,:,pic_num), xiao_cankao(2,:,pic_num), 'green', 'LineWidth', 0.5); hold on;
    plot(movmean(xiao_tuiyan(1,:,pic_num),10), xiao_tuiyan(2,:,pic_num), 'magenta', 'LineWidth', 0.5); hold off;

    ylabel('Orbital radius (km)');
    xlabel('Probability density');
    legend('Reference distribution','Smoothed inferred distribution','Location','northeast');
...
KL_sanduPQ=zeros(1,ns+1);
KL_sanduQP=zeros(1,ns+1);
JS_sandu_smooth=zeros(1,ns+1);
for i=1:ns+1
    P_KL=xiao_cankao(1,:,i);
    Q_KL=movmean(xiao_tuiyan(1,:,i),10);
    for j=1:(hi_fenbu-lo_fenbu)/dR_fenbu
        if(P_KL(j)~=0)
            KL_sanduPQ(i)=KL_sanduPQ(i)+P_KL(j)*log(P_KL(j)/Q_KL(j));
    ...
    if(Px_cankao(i,1)~=0)
        KL_sanduPQ(i)=KL_sanduPQ(i)+Px_cankao(i,1)*log(Px_cankao(i,1)/Px_tuiyan(i,1));
        JS_sandu_smooth(i)=JS_sandu_smooth(i)+0.5*Px_cankao(i,1)*log(Px_cankao(i,1)*2/(Px_cankao(i,1)+Px_tuiyan(i,1)));
    end
    if(Px_cankao(i,2)~=0)
        KL_sanduPQ(i)=KL_sanduPQ(i)+Px_cankao(i,2)*log(Px_cankao(i,2)/Px_tuiyan(i,2));
        JS_sandu_smooth(i)=JS_sandu_smooth(i)+0.5*Px_cankao(i,2)*log(Px_cankao(i,2)*2/(Px_cankao(i,2)+Px_tuiyan(i,2)));
    end
    if(Px_tuiyan(i,1)~=0)
        KL_sanduQP(i)=KL_sanduQP(i)+Px_tuiyan(i,1)*log(Px_tuiyan(i,1)/Px_cankao(i,1));
        JS_sandu_smooth(i)=JS_sandu_smooth(i)+0.5*Px_tuiyan(i,1)*log(Px_tuiyan(i,1)*2/(Px_cankao(i,1)+Px_tuiyan(i,1)));
    end
    if(Px_tuiyan(i,2)~=0)
        KL_sanduQP(i)=KL_sanduQP(i)+Px_tuiyan(i,2)*log(Px_tuiyan(i,2)/Px_cankao(i,2));
        JS_sandu_smooth(i)=JS_sandu_smooth(i)+0.5*Px_tuiyan(i,2)*log(Px_tuiyan(i,2)*2/(Px_cankao(i,2)+Px_tuiyan(i,2)));
    end
end
...
plot(t_days, JS_sandu_smooth,'Color', [230, 90, 13]/255, 'LineWidth', 1); hold off;
xlabel('Evolution time (day)');
ylabel('JS divergence');
legend('Before smoothing','After smoothing','Location','northeast');
ylim([0 0.05]);
...
outfile = fullfile(pwd,'fig1.png');  % 路径可自改
